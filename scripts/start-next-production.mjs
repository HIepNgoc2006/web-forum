...
import { createProductionFrontProxy } from '../backend/dist/src/server/front-proxy.js';
...
function shutdown(code, signal, reason) {
  if (stopping) return;
  stopping = true;
  shutdownCode = code;
...
function supervise(label, command, args, options) {
  const child = {
    label,
