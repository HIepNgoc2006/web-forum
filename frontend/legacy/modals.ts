...
import { mountComposerFormatToolbars } from './composer-format';
  ...
  return function showPostEditModal(globalNumber, initialBody = '', options: AnyRecord = {}) {
    return new Promise<any>((resolve) => {
      ...
      <div class="reason-modal post-edit-modal" role="dialog" aria-modal="true" aria-labelledby="postEditModalTitle">
        ...
        <div class="composer-body-wrap post-edit-body-wrap">
    ...
    `;
      ...
      // Attach the same BBCode toolbar used by composers (bold/italic/lists/preview/etc.).
      mountComposerFormatToolbars();

      const bodyTextarea = overlay.querySelector('#postEditTextarea') as HTMLTextAreaElement | null;
      const reasonTextarea = overlay.querySelector('#postEditReasonTextarea') as HTMLTextAreaElement | null;
      const keepImagesInput = overlay.querySelector('#postEditKeepImages') as HTMLInputElement | null;
      const fileInput = overlay.querySelector('#postEditFileInput') as HTMLInputElement | null;
      const spoilerInput = overlay.querySelector('#postEditSpoiler') as HTMLInputElement | null;
      const preview = overlay.querySelector('#postEditPreview') as HTMLElement | null;
      const confirmBtn = overlay.querySelector('#postEditConfirmBtn') as HTMLButtonElement | null;
      const cancelBtn = overlay.querySelector('#postEditCancelBtn') as HTMLButtonElement | null;
      let selectedMedia: AnyRecord[] = [];
      let settled = false;
