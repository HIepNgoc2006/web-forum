...
type AccountPreferences = {
  ...
  setHomeSyncCallbacks: (callbacks: {
    syncBoardSubscriptionButtons?: () => void;
    syncBoardHiddenButtons?: () => void;
...
export function createAccountPreferencesController({
  ...
  function applyDisplayPreferences(preferences = localDisplayPreferences()) {
    ...
    document.documentElement.dataset.fontSize = safe.fontSize || 'medium';
    ...
    if (els?.accountFontSize) {
      els.accountFontSize.value = safe.fontSize || 'medium';
    }
  ...
  function syncAccountHiddenBoardOptions(settings = state.account?.settings || accountSettingsFromLocal()) {
    if (!els.accountHiddenBoards) {
      return;
    }
    const selected = new Set(
      Array.isArray(settings.hiddenBoards)
        ? settings.hiddenBoards.map(String)
        : [...hiddenBoardSlugs()]
    );
    els.accountHiddenBoards.innerHTML = state.boards
      .map((board) => {
        const safeSlug = escapeHtml(board.slug);
        const inputId = `accountHiddenBoard-${safeSlug}`;
        return `
        <label for="${inputId}">
          <input id="${inputId}" name="hiddenBoards" type="checkbox" value="${safeSlug}" data-account-hidden-board ${
            selected.has(board.slug) ? 'checked' : ''
          } />
          <span>${escapeHtml(board.path)} ${escapeHtml(board.name)}</span>
        </label>
      `;
      })
      .join('');
  ...
  function applyAccountSyncedSettings(account = state.account) {
    ...
    writeHiddenBoardSlugs(Array.isArray(settings.hiddenBoards) ? settings.hiddenBoards : []);
    homeSyncCallbacks.syncBoardSubscriptionButtons();
    homeSyncCallbacks.syncBoardHiddenButtons();
    syncAccountBoardSubscriptionOptions(settings);
    syncAccountHiddenBoardOptions(settings);
    homeSyncCallbacks.renderBoards();
  ...
  return {
    ...
    setHomeSyncCallbacks: (callbacks: {
      ...
      homeSyncCallbacks = {
        syncBoardSubscriptionButtons:
          callbacks.syncBoardSubscriptionButtons || homeSyncCallbacks.syncBoardSubscriptionButtons,
        syncBoardHiddenButtons: callbacks.syncBoardHiddenButtons || homeSyncCallbacks.syncBoardHiddenButtons,
        renderSubscribedBoards: callbacks.renderSubscribedBoards || homeSyncCallbacks.renderSubscribedBoards,
        renderBoards: callbacks.renderBoards || homeSyncCallbacks.renderBoards
      };
    }
  };
}
