...
import { handlePostLinkEmbedClick } from './link-preview';
...
export function bindThreadEvents(dependencies: ThreadEventDependencies) {
  ...
  body.addEventListener('click', async (event) => {
    ...
    if (handlePostLinkEmbedClick(event)) {
      return;
    }
    ...
    if (target.closest('[data-topbar-hidden-menu-close]')) {
      els.topbarHiddenThreadsMenu?.removeAttribute('open');
    }
