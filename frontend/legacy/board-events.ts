...
export function handleBoardCatalogControlClick(
  ...
  const boardHiddenButton = target.closest('[data-toggle-board-hidden]');
  if (boardHiddenButton) {
    return (async () => {
      if (typeof toggleBoardHidden === 'function') {
        await toggleBoardHidden();
      }
      if (typeof syncBoardHiddenButtons === 'function') {
        syncBoardHiddenButtons();
      }
    })();
  }
