...
import {
  ...
  notifySubscribedBoardThread as notifySubscribedBoardThreadWithDependencies,
  notifyTaggedPost as notifyTaggedPostWithDependencies,
...
const {
  ...
  setAccountSession: setAccountSessionBase,
  isCapcodeEligible,
  updateCapcodeOptions,
  updateAccountDisplayOptions
} = accountPreferencesController;
let syncDmAccountSession = () => {};
function setAccountSession(session: AnyRecord = {}) {
  setAccountSessionBase(session);
  syncDmAccountSession();
}
...
accountPreferencesController.setHomeSyncCallbacks({
  ...
  syncBoardHiddenButtons: homeController.syncBoardHiddenButtons,
  renderSubscribedBoards,
  renderBoards: homeController.renderBoards
});
...
syncDmAccountSession = () => dmController.updateDmNavVisibility();
...
function isViewingNotificationThread(threadId: string) {
  return (
    document.visibilityState === 'visible' &&
    (window.location.hash || '').startsWith('#thread/') &&
    String(state.threadId || '') === threadId
  );
}

function notifyTaggedPost(payload: AnyRecord = {}) {
  return notifyTaggedPostWithDependencies(payload, {
    accountUsername: state.account?.username,
    browserNotificationIds: state.browserNotificationIds,
    isOwnPost: isMyPost,
    isViewingThread: isViewingNotificationThread
  });
}

function notifySubscribedBoardThread(payload: AnyRecord = {}) {
  const tagged = notifyTaggedPost(payload);
  notifySubscribedBoardThreadWithDependencies(payload, {
    browserNotificationIds: state.browserNotificationIds,
    isBoardSubscribed: (boardSlug: string) => subscribedBoardSlugs().has(boardSlug),
    isOwnPost: isMyPost,
    isViewingBoard: (boardSlug: string) =>
      document.visibilityState === 'visible' &&
      String(state.boardSlug || '') === boardSlug &&
      /^#(?:board|catalog|archive)\//.test(window.location.hash || ''),
    suppressBrowserNotification: tagged
  });
}
...
export const appReady = bootstrapApp({
  els,
  bindEvents,
  ...
  toggleBoardHidden: homeController.toggleBoardHidden,
  loadAdmin,
  closeQuickReply,
  syncAccountHomeBoardOptions,
  loadAccountSession,
