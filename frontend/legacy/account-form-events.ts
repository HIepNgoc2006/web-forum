...
export function bindAccountFormEvents({
  ...
  async function submitAccountSettings(event) {
    ...
    const displayPreferences = writeLocalDisplayPreferences({
      ...
      fontSize: els.accountFontSize?.value || 'medium'
    });
    const wantsBrowserNotify =
      Boolean(els.accountBrowserNotifyWatchedThreads?.checked) ||
      Boolean(els.accountBrowserNotifyBoardSubscriptions?.checked) ||
      Boolean(els.accountBrowserNotifyMentions?.checked) ||
    ...
    const notificationPreferences = writeLocalNotificationPreferences({
      ...
      emailMentions: Boolean(els.accountEmailNotifyMentions?.checked),
      emailDirectMessages: Boolean(els.accountEmailNotifyDirectMessages?.checked),
      ...
      browserBoardSubscriptions:
        Boolean(els.accountBrowserNotifyBoardSubscriptions?.checked) && browserPermissionOk,
      browserMentions: Boolean(els.accountBrowserNotifyMentions?.checked) && browserPermissionOk,
    ...
    const hiddenBoards = els.accountHiddenBoards
      ? [...els.accountHiddenBoards.querySelectorAll('[data-account-hidden-board]:checked')].map((input) => input.value)
      : [];
    ...
    writeHiddenBoardSlugs?.(hiddenBoards);
    syncBoardSubscriptionButtons();
    syncBoardHiddenButtons?.();
    try {
      if (!state.accountToken || !state.account) {
        applyAccountSyncedSettings();
        fillAccountSettings();
        showToast('Đã lưu cài đặt trên trình duyệt này.');
        return;
      }
      ...
      fillAccountSettings(account);
      updateAccountNav();
      showToast('Đã lưu cài đặt tài khoản.');
    } catch (error) {
