...
const COMPOSER_TARGETS = new Set(['thread', 'comment', 'quickReply', 'dm', 'dmEdit']);
...
/** Set by bindComposerMediaPicker so other modules (e.g. DM) can hydrate media tokens. */
let mediaHydrationScheduler: (() => void) | null = null;
/** Set by bindComposerMediaPicker so DM can close the picker before re-rendering edit forms. */
let mediaPickerCloser: ((options?: { restoreFocus?: boolean }) => void) | null = null;

export function requestComposerMediaHydration(): void {
  mediaHydrationScheduler?.();
}

export function closeComposerMediaPicker(options?: { restoreFocus?: boolean }): void {
  mediaPickerCloser?.(options);
}
...
export function bindComposerMediaPicker({ api, insertComposerToken, showToast }: AnyRecord): () => void {
  ...
  mediaHydrationScheduler = scheduleHydration;
  mediaPickerCloser = closePicker;
  ...
  return () => {
    if (mediaHydrationScheduler === scheduleHydration) {
      mediaHydrationScheduler = null;
    }
    if (mediaPickerCloser === closePicker) {
      mediaPickerCloser = null;
    }
    requestController?.abort();
